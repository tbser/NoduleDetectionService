...
def get_normalized_img_float(img):
    img = img.astype(numpy.float)
    min = img.min()
    max = img.max()
    img -= min
...
def rescale_patient_images(images_zyx, org_spacing_xyz, target_voxel_mm, is_mask_image=False, verbose=False, logger=None):
    ...
    logger.info("Shape after: {0} ".format(res.shape))
    return res
...
def load_patient_dicom(src_dir):
    lstFilesDCM = []
    ...
    if not hasattr(slices[0], 'SliceThickness'):
        try:
